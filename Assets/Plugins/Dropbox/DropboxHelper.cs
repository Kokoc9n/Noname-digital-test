...
namespace Plugins.Dropbox
{
    public static class DropboxHelper
    {
        ...
        public static async Task DownloadAndSaveFile(string relativePathToFile, IProgress<float> progress = null, Action onComplete = null)
        {
            ...
            while (!operation.isDone)
            {
                ...
                progress?.Report(downloadRequest.downloadProgress);
            }

            if (downloadRequest.result != UnityWebRequest.Result.Success)
            {
                Debug.LogError("Failed to download file at path: " + relativePathToFile + " " + downloadRequest.error);
            }
            else
            {
        ...
        public static async Task DownloadFilesInFolder(string path, IProgress<float> progress = null, Action onComplete = null)
        {
            Debug.Log("--- Files ---");
            var list = await _client.Files.ListFolderAsync(path);

            float totalFiles = list.Entries.Count(i => i.IsFile);
            float filesDownloaded = 0;

            //foreach (var item in list.Entries.Where(i => i.IsFolder))
            //{
            //    Debug.Log("D  {0}/" + item.Name);
            //}

            foreach (var item in list.Entries.Where(i => i.IsFile))
            {
                var file = item.AsFile;
                string filePath = Application.persistentDataPath + file.PathDisplay;
                if (File.Exists(filePath) == false)
                {
                    Debug.Log("new file " + item.Name);
                    await DownloadAndSaveFile(item.PathLower.TrimStart('/'));
                    filesDownloaded++;
                    progress?.Report(filesDownloaded / totalFiles);
                }
            }
